...
export default (nodes) => {
  ...
  let mappedData = nodes.map((obj, i) => {
    ...
    const sameTagIndex =
      i + document.getElementsByTagName(obj.localName).length;
    let children = [];
    ...
    if (output.tag === "text" || output.tag === "tspan") {
      output.props.textContent = obj.childNodes.length
    ...
    if (!obj["data-react-d3-id"]) {
      output["data-react-d3-id"] =
        output.tag + "." + sameTagIndex + "." + 0 + "." + 0;
